...
Requires **Python 3.11+** (stdlib `tomllib`) **or** **Python 3.9–3.10** with the
`tomli` package installed (`pip install -r _bmad/scripts/requirements.txt`).
No virtualenv is required if your `python3` already meets either condition.
...
try:
    ...
    try:
        ...
        sys.stderr.write(
            ...
            "  Option A — Python 3.11+: install from python.org or Homebrew, then use e.g.\n"
            "    python3.11 _bmad/scripts/resolve_customization.py ...\n"
            "  Option B — Keep Python 3.9/3.10: install the backport once:\n"
            "    pip3 install -r _bmad/scripts/requirements.txt\n"
            "  Option C — Run the merge manually per the skill's SKILL.md.\n"
        )
        sys.exit(3)
